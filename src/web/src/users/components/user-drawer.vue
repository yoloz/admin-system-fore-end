...
const deleteUser = (id: number) => {
    if (id) {
        ElMessageBox.confirm('确认删除用户吗?', '删除', { confirmButtonText: '确认', cancelButtonText: '取消', type: 'warning' }).then(() => {
...
<template>
    <el-drawer class="hrm-drawer" v-model="detailsModel" :show-close="false" size="600px">
        <template #header="{ close, titleId, titleClass }">
            ...
            <el-button style="margin-right: 15px;" :icon="Edit" type="primary" @click="edit"
            ...
            <el-dropdown @command="handleCommand">
                ...
                <template #dropdown>
                    <el-dropdown-menu>
                        <el-dropdown-item command="disabled" v-if="userDetail?.enable">停用</el-dropdown-item>
                        <el-dropdown-item command="activation" v-else>启用</el-dropdown-item>
        ...
        <div class="drawer-body">
            <div class="drawer-body-header">
                <p>账号状态:
                    <span class="status success" style="margin-left:15px" v-if="userDetail?.enable">启用</span>
                    <span class="status pending" style="margin-left:15px" v-else>停用</span>
                </p>
                <p>创建时间: <span style="margin-left:15px">{{ dayjs(userDetail?.createTime).format('YYYY-MM-DD HH:mm:ss')
                }}</span></p>
            </div>
            ...
            <li class="list-item">
                ...
                <p>
                    <el-tag type="info" text v-for="item in userDetail?.roles" :key="item.id" round>{{ item.name }}</el-tag>
                </p>
            ...
            <li class="list-item">
                ...
                <span class="value" v-if="userDetail?.loginTime">{{ dayjs(userDetail.loginTime).format('YYYY-MM-DD HH:mm:ss')
                }}</span>
    ...
    <reset-pwd-dialog ref="resetPwdDialogRef" @refresh-table="updateTable"></reset-pwd-dialog>
    <create-user-dialog ref="createUserDialogRef" @refresh-table="updateTable"></create-user-dialog>
    <user-role-dialog ref="userRoleDialogRef" @refresh-table="updateTable"></user-role-dialog>
</template>
